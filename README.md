...
3. **APIのアクセス許可** に以下を追加（アプリケーション権限）し、管理者の同意を付与：
   ...
   - `User.Read.All`
   - `Sites.Read.All`
4. **証明書とシークレット** → **証明書のアップロード** で認証用証明書を登録し、証明書の拇印（Thumbprint）をコピー
5. **概要** から **アプリケーション (クライアント) ID** と **ディレクトリ (テナント) ID** をコピー

### 2. 接続設定

設定はアプリ内の **設定画面** から入力します。入力した値は Windows レジストリ（`HKCU\Software\OneDriveAccessGuard`）に保存されます。

| 項目 | 説明 |
|------|------|
| クライアント ID | Azure App Registration のアプリケーション (クライアント) ID |
| テナント ID | Azure App Registration のディレクトリ (テナント) ID |
| 証明書の拇印 | 登録した証明書の Thumbprint |
